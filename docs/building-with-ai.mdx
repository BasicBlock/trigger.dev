...
Always import from `@basicblock/trigger-sdk` — never from `@basicblock/trigger-sdk/v3` or use the deprecated `client.defineJob` pattern.

## Task pattern

Every task must be exported. Use `task()` from `@basicblock/trigger-sdk`:
...
```ts
import { wait } from "@basicblock/trigger-sdk";
...
```ts
import { defineConfig } from "@basicblock/trigger-sdk/build";
...
2. **Importing from `@basicblock/trigger-sdk/v3`** — this is the old v3 path; always use `@basicblock/trigger-sdk`
