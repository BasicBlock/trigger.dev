...
If you need to execute Python scripts in your Trigger.dev project, you can use the `pythonExtension` build extension via the `@basicblock/trigger-python` package.

First, you'll need to install the `@basicblock/trigger-python` package:

```bash
npm add @basicblock/trigger-python
```
...
This will take care of adding python to the build image and setting up the necessary environment variables to execute Python scripts. You can then use our `python` utilities in the `@basicblock/trigger-python` package to execute Python scripts in your tasks. For example, running a Python script inline in a task:
