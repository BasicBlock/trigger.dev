...
You can also extract that out into a function instead of defining it inline, in which case you will need to import the `BuildExtension` type from the `@basicblock/trigger-build` package:

<Note>
  You'll need to add the `@basicblock/trigger-build` package to your `devDependencies` before the below
  code will work. Make sure it's version matches that of the installed `@basicblock/trigger-sdk` package.
</Note>

```ts
import { defineConfig } from "@basicblock/trigger-sdk";
import { BuildExtension } from "@basicblock/trigger-build";
