...
Build extension allow you to hook into the build system and customize the build process or the resulting bundle and container image (in the case of deploying). You can use pre-built extensions by installing the `@basicblock/trigger-build` package into your `devDependencies`, or you can create your own.
