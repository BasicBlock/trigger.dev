...
```ts
import { eventTrigger } from "@basicblock/trigger-sdk";
...
2. v2 had integrations with some APIs. Any package that isn't `@basicblock/trigger-sdk` can be replaced with an official SDK. The syntax may need to be adapted.
For example:
v2:
...
4. The import for `task` in v3 is `import { task } from "@basicblock/trigger-sdk";`
