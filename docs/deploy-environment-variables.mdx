...
```ts
import { envvars } from "@basicblock/trigger-sdk";
...
```ts
import { envvars, task } from "@basicblock/trigger-sdk";
import { readFileSync } from "fs";
import { parse } from "dotenv";
...
<Note>
  To use the `syncEnvVars` build extension, you should first install the `@basicblock/trigger-build`
