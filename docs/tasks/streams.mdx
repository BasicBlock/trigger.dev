...
<Note>
  Streams v2 requires SDK version **4.1.0 or later**. Make sure to upgrade your `@basicblock/trigger-sdk`
  and `@basicblock/trigger-react-hooks` packages to use these features. If you're on an earlier version,
...
```ts
import { streams, task } from "@basicblock/trigger-sdk";
...
```ts
// Before (v1)
import { metadata, task } from "@basicblock/trigger-sdk";
